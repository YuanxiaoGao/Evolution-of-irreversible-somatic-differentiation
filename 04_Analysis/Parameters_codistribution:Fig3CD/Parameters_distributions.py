...
dfData.columns = ['Maturity.size', 'Differentiation.cost', 'Line.ID', 'Label', 'Prob.Soma2Soma',
			   'g_ss', 'g_gs', 'g_gg', 'alpha', 'x_0', 'b', 'x_1', 'Size.costs']
...
plt.scatter(ISD_Data[ParX], ISD_Data[ParY], s=size_focal, c=CLR_ISD, alpha=1.0,label="sth")

if ParY == 'alpha':
	ax.set_yticks([-2, -1, 0, 1, 2])
	ax.set_yticklabels(['$10^{-2}$', '$10^{-1}$', '$10^{0}$', '$10^{1}$', '$10^{2}$'])
	Ylabel = "Contribution synergy, "+r"$\alpha$"
elif ParY == 'b':
...
if Write2File == 1:
	fig.savefig('Correlation_ISD_'+str(ParX)+'_vs_'+str(ParY)+'_v2.pdf',bbox_inches='tight')   # save figures
